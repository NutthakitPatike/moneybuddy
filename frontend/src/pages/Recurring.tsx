...
export function Recurring() {
  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-black">รายการประจำ</h1><p className="text-cocoa/65">ตั้งรายรับรายจ่ายที่เกิดซ้ำ และรันผ่าน backend/scheduler ใน production</p></div><div className="flex gap-2"><Button variant="secondary" onClick={runDue}>รันรายการครบกำหนด</Button><Button onClick={() => start()}><Plus size={18} /> เพิ่ม</Button></div></div>
      <div className="grid gap-3">
        {items.map((item) => (
          <Card key={item.id} className="flex flex-wrap items-center justify-between gap-3">
            <div><p className="font-bold">{item.title}</p><p className="text-sm text-cocoa/60">{categoryName(categories, item.category_id)} · {item.frequency} · next {item.next_run_date}</p></div>
